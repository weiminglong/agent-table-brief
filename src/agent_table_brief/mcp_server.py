...
def _handle_error(e: Exception) -> dict[str, Any]:
    if isinstance(e, RepoNotScannedError):
        return McpError("repo_not_scanned", str(e), {}).to_dict()
    if isinstance(e, RepoAmbiguousError):
        return McpError("repo_ambiguous", str(e), {}).to_dict()
    if isinstance(e, KeyError):
        return McpError("brief_not_found", str(e), {}).to_dict()
    if isinstance(e, ValueError) and "ambiguous" in str(e).lower():
        return McpError("brief_ambiguous", str(e), {}).to_dict()
    return McpError("mcp_error", str(e), {"type": type(e).__name__}).to_dict()


@mcp.tool()
def search_tables(query: str, repo: str | None = None, limit: int = 10) -> dict[str, Any]:
...
@mcp.tool()
def get_brief(table: str, repo: str | None = None) -> dict[str, Any]:
...
@mcp.tool()
def compare_tables(tables: list[str], repo: str | None = None) -> dict[str, Any]:
...
@mcp.tool()
def list_tables(repo: str | None = None) -> dict[str, Any]:
...
def get_catalog(repo_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        if match is None:
            return McpError("repo_not_found", f"Repository not found: {repo_key}", {}).to_dict()
